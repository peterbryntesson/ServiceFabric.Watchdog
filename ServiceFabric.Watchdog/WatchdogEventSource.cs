...
{
    [EventSource(Name = "ServiceFabric.Watchdog")]
    internal sealed class WatchdogEventSource : EventSource
    {
        public static readonly WatchdogEventSource Current = new WatchdogEventSource();

        static WatchdogEventSource()
        {
        ...
        private WatchdogEventSource() : base() { }
        ...
        {
            public const EventKeywords Triggers = (EventKeywords)0x1L;
            public const EventKeywords Actions = (EventKeywords)0x2L;
            public const EventKeywords Rules = (EventKeywords)0x4L;
        }
        #endregion
        ...
        private const int TriggerItemReceivedId = 3;
        [Event(TriggerItemReceivedId, Level = EventLevel.Informational, Message = "Trigger item {0} received", Keywords = Keywords.Triggers)]
        public void TriggerItemReceived(string triggerItem)
        {
            WriteEvent(TriggerItemReceivedId, triggerItem);
        }

        private const int RuleFiredId = 4;
        [Event(RuleFiredId, Level = EventLevel.Informational, Message = "Rule {0} fired for trigger item {1}. Result was {2}", Keywords = Keywords.Rules)]
        public void RuleFired(string rule, string triggerItem, bool triggered)
        {
            WriteEvent(RuleFiredId, rule, triggerItem, triggered);
        }

        private const int ActionFiredId = 5;
